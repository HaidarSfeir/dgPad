...
namespace CmsShoppingCart.Infrastructure
{
    public class PaginationTagHelper : TagHelper
    {
        ...
        private void ClosingHtml(StringBuilder content)
        {
            ...
            {
                content.Append($"<li class='page-item'><a class='page-link' " +
                    $"href='{PageTarget}?p={PageCount}'>{PageLast}</a></li>");
            }
            content.Append(" </ul");
        }
        ...
        private void InitPaginationHtml(StringBuilder content)
        {
            ...
            if (PageNumber != 1)
            {
                content.Append($"<li class='page-item'><a class='page-link' href='{PageTarget}?p={PageFirst}'>{PageFirst}</a></li>");
            }
        }
